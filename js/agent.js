/* ═══════════════════════════════════════════════════════
   ...
   Claude AI chat: agentic loop com tool_result correto
═══════════════════════════════════════════════════════ */
...
const AGENT_TOOLS = [
  ...
  { name:"github_create_or_update_file", description:"[ALTO RISCO] Cria ou atualiza um arquivo no GitHub com commit.", input_schema:{ type:"object", properties:{ path:{type:"string"}, content:{type:"string"}, message:{type:"string"}, branch:{type:"string"} }, required:["path","content","message"] } },
...
function buildSystemPrompt() {
  ...
  return `Você é o Claw DevOps Agent — agente autônomo de infraestrutura.
...
- Tools HIGH RISK: enfileire para aprovação humana
  ...
  try {
    switch(toolName) {
      ...
      case "render_get_logs": {
        ...
        if (!res.ok) return "⚠ Render logs: HTTP " + res.status + " — API pode não suportar este endpoint no plano atual.";
      ...
      case "analyze_error":
        return `Análise do erro: "${input.error_message}"\n\nPossíveis causas:\n- Acesso a propriedade de objeto null/undefined\n- Variável não inicializada antes do uso\n\nSugestão: use optional chaining (?.) e verifique se o objeto existe antes de acessar suas propriedades.\n\nPara análise detalhada, leia o arquivo onde o erro ocorre com github_read_file.`;
      ...
      case "edit_file":
        ...
        return `✅ Arquivo editado localmente: ${input.path}\n${input.description}\n\nClique em ⬆ COMMIT na aba Editor para subir ao GitHub.`;
      default:
...
async function sendAgent() {
  ...
  try {
    ...
    for (let iter = 0; iter < MAX_ITER; iter++) {
      ...
      // 4. Se stop_reason é end_turn ou não há tools, termina
      if (data.stop_reason === "end_turn" || toolUses.length === 0) break;

      // 5. Executa todas as tools e coleta resultados
      const toolResults = [];
      ...
      for (const tu of toolUses) {
        ...
        if (isHigh) {
          ...
          toolResult = "Ação enfileirada para aprovação humana.";
        ...
        // CRÍTICO: sempre adiciona tool_result para cada tool_use
        toolResults.push({
          type: "tool_result",
          tool_use_id: tu.id,
          content: toolResult
      ...
      // 6. Adiciona todos os tool_results no histórico antes do próximo turno
      STATE.agentHistory.push({ role:"user", content: toolResults });

      // 7. Se todos eram high-risk, para — não adianta continuar sem resultado real
      if (toolUses.every(tu => HIGH_RISK_TOOLS_SET.has(tu.name))) break;
    }
    ...
    STATE.agentMsgs.push({ role:"assistant", text:"Erro: " + err.message + "\n\nVerifique: API key configurada? (⚙ API KEYS)" });
